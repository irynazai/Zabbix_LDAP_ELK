...
variable "network" {
    description 			= "The name of the apache network."
    default     			= "vpc-network"
}


variable "apache_subnet" {
    description 			= "The name or self_link of the apachesubnetwork to attach this interface to. The subnetwork must exist in the same region this instance will be created in. Either network or subnetwork must be provided."
    default     			= "apache-subnet"
}


variable "apache_subnet_cidr" {
    description 			= "The apache IP range od address to assign to the instance. If empty, the address will be automatically assigned."
    default     			= "10.0.2.0/24"
}
...
variable "apache_tags" {
  description 				= "A tags of db host."
  default     				= ["apache-tag","http-server"]    
}
