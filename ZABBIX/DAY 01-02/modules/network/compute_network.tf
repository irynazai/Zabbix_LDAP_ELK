...
resource "google_compute_address" "static-server-ip" {
...
resource "google_compute_firewall" "deny-internal" {
    ...
    target_tags             	= [var.server_tag, var.ldap_tag, var.client_tag]
    source_ranges             	= var.jump_ip 
}
...
resource "google_compute_firewall" "allow-http" {
    ...
    allow {
        ...
        ports                   = ["80","389","443","587","3306","8080","10051","10050"]
    }
    target_tags                 = [var.server_tag, var.ldap_tag, var.client_tag]
